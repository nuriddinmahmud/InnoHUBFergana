...
const Register = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (strength < 2) {
      setError("Parol kamida 10 ta belgiga ega bo'lishi kerak");
      return;
    }
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Ro'yxatdan o'tishda xato");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-background flex items-center justify-center matrix-bg">
      ...
      <div className="relative z-10 w-full max-w-[480px] mx-4">
        <div className="bg-card border border-border rounded-2xl p-12">
          ...
          <h2 className="text-[28px] font-bold text-center mb-2">Ro'yxatdan o'ting</h2>
          <p className="text-muted-foreground text-center text-[15px] mb-8">Bepul hisob yarating</p>

          {/* Google */}
          <button type="button" className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-lg bg-secondary border border-border text-foreground text-[15px] font-medium hover:bg-secondary/80 transition-colors mb-6" title="Google bilan ro'yxatdan o'tish">
          ...
          <p className="text-center mt-6 text-sm text-muted-foreground">
            Hisobingiz bormi?{" "}
            <Link to="/login" className="text-primary hover:underline font-medium">Kiring</Link>
          </p>
        </div>
      </div>
    </div>
